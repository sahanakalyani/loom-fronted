...
const BADGE = {
  ...
  reply: {
    bg: "bg-blue-50",
    icon: <MessageCircle size={13} className="text-gray-500" strokeWidth={2} />,
  },
};
const Activity = () => {
      ...
      <div className="w-full md:bg-white md:border md:border-black/10 md:rounded-3xl md:shadow-xs">
        ...
        {!loading && !error && !items.length && (
          <div className="flex flex-col items-center justify-center px-6 py-16 gap-3 text-center">
            ...
            <p className="text-sm text-gray-400 leading-relaxed max-w-xs">
              When someone likes, replies or follows you, it will show up here.
            </p>
          </div>
        )}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[14px] text-gray-900 leading-snug ">
                      ...
                      {item.type === "like" && " likedyourpost"}
                      {item.type === "follow" && " followedyou"}
                      {item.type === "reply" && " repliedtoyourthread"}
                    </p>
                    {item.created_at && (
                  ...
                  {item.type == "reply" ? (
                    ...
                    <div
                      className={`w-8 h-8 min-w-[32px] roundedd-full ${badge.bg} flex items-center justify-center`}
                    >
                      {badge.icon}
                    </div>
                  )}
